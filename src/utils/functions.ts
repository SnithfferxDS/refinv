...
import type { Quotation, Quotation_Details } from "../types/Quotation";
...
public function Importes(datos: Quotation_Details) {
    ...
    if (datos.length > 0) {
        ...
        // }
        /*------------------ Asignación de datos -----------------*/
        let price = parseFloat(datos.price);
        let weight = parseFloat(datos.weight);
        let quantity = parseInt(datos.quantity);
        let store = parseInt(datos.store);
        let country = parseInt(datos.country);
        let shipping = parseFloat(datos.shipping);
        let category = parseInt(datos.category);
        let product = this -> scapeChars(datos.product);
        let link = datos.enlace;
        let product_model = (!isset(datos.modelo) || empty(datos.modelo)) ? "" : datos.modelo;
        let image = datos.image;
        let date = DateTime:: createFromFormat("d/m/Y h:i:s a", datos.date);
        let ship_date = date -> format('Y-m-d H:i:s');
        ...
        if (!empty($price) || $price > 0) {
            ...
            if ($store != '' AND $store > 0) {
                ...
                $taxes = $this -> get_Impuestos(($price * $quantity), $store, "TAX");
                $costo = (($price * $quantity) + ($shipping * $quantity)) + $taxes;
                switch ($mesure) {
                    case 'gr':
                    ...
                    case 'lb':
                        ...
                        break;
                    case 'kb':
                        $kilogramos = $weight * ($this -> amular((8 ^ 6)));
                        $libras = $weightTotal * 0.002;
                        break;
                    case 'Mb':
                        $kilogramos = $weight * ($this -> amular((1024 ^ 2)));
                        $libras = $weightTotal * 0.002;
                        break;
                    case 'Gb':
                        $kilogramos = $weight * ($this -> amular((1024 ^ 3)));
                        $libras = $weightTotal * 0.002;
                        break;
                    default:
                        $kilogramos = $weight;
                ...
                if ($category == 3 || $category == "3") {
                    $tiempo_Envio = $this -> calcula_dias($hoy, $ship_date, 20);
                } else {
                    $tiempo_Envio = $this -> calcula_dias($hoy, $ship_date, 10);
                }
                /*----------- Calculando flete y adicionales ------------*/
                $montoMod = $this -> descuento($costo);
                switch ($currier) {
                    case 'trans':
                        $tramites = $this -> trans($libras, 'false');
                        $tmts2 = $this -> aero($libras, $montoMod);
                        $tmts3 = $this -> cargo($libras);
                        $curriers = [
                            ['name'=> "Transexpress"],
                            ['name'=> "AeroPost"],
                        ...
                        break;
                    case 'aero':
                        $tramites = $this -> aero($libras, $montoMod);
                        $tmts2 = $this -> trans($libras, 'false');
                        $tmts3 = $this -> cargo($libras);
                        $curriers = [
                            ['name'=> "AeroPost"],
                            ['name'=> "Transexpress"],
                        ...
                        break;
                    case 'cargo':
                        $tramites = $this -> cargo($libras);
                        $tmts2 = $this -> trans($libras, 'false');
                        $tmts3 = $this -> aero($libras, $montoMod);
                        $curriers = [
                            ['name'=> "Cargo (consolidado)"],
                            ['name'=> "Transexpress"],
                            ['name'=> "AeroPost"],
                        ...
                        break;
                    case 'digit':
                        $tramites = $this -> digital($costo, $libras);
                        $tmts2 = $this -> trans($libras, 'false');
                        $tmts3 = $this -> aero($libras, $montoMod);
                        $curriers = [
                            ['name'=> "Cargo Digital"],
                        ...
                        break;
                    default:
                        $tramites = $this -> trans($libras, 'true');
                        $tmts2 = $this -> aero($libras, $montoMod);
                        $tmts3 = $this -> cargo($libras);
                        $curriers = [
                            ['name'=> "Transexpress"],
                ...
                if ($category != '' AND $category > 0) {
                    $daiBase = $this -> get_Impuestos($valorCIF, $category, 'DAI');
                    if (!empty($country) AND $country > 0) {
                        if (is_string($daiBase)) {
                            ...
                            $dai = ($daiBase > 0) ? ($this -> get_Impuestos($daiBase, $country, 'TLC') / $quantity) : 0.0;
                        }
                    } else {
                        $dai = 0.00;
                ...
                $ganancia = $this -> cal_ganancia($costods);
                ...
                if ($costo_Cescc_CF > 0 AND $costo_Cescc_CM > 0 AND $costo_Cescc_CG > 0) {
                    if ($category != '' AND $category > 0) {
                        //Calculando el ces.
                        $cesccCF = $this -> get_Impuestos($costo_Cescc_CF, $category, 'CES'); //CESCC Consumidor
                        $cesccCM = $this -> get_Impuestos($costo_Cescc_CM, $category, 'CES'); //CESCC Mayoreo
                        $cesccCG = $this -> get_Impuestos($costo_Cescc_CG, $category, 'CES'); //CESCC Gobierno
                ...
                if ($error_CES == '' and $error_DAI == '' and $error_Tax == '') {
                    ...
                    $view = $this -> view('Cotizaciones/Cotizado', $cotizado);
                    $mailData = [
